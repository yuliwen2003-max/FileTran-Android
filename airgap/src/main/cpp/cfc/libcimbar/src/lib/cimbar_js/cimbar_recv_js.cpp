...
int cimbard_scan_extract_decode(const uchar* imgdata, unsigned imgw, unsigned imgh, int format, uchar* bufspace, unsigned bufsize)
{
	...
	_reporting = fmt::format("sce: {}, imgdec: {}, fmt={}, {}x{}", _tScanExtract.avg(), _tImgDecode.avg(), format, imgw, imgh);

	bool shouldPreprocess = true;
	int extractRes = 0;
	{
		Timer t(_tScanExtract);
		extractRes = ext.extract(img, img);
		if (!extractRes)
			return -3;
		else if (extractRes == Extractor::NEEDS_SHARPEN)
	...
	_reporting = fmt::format(
		"sce: {}, imgdec: {}, fmt={}, {}x{}, ext={}, decoded_bytes={}",
		_tScanExtract.avg(),
		_tImgDecode.avg(),
		format,
		imgw,
