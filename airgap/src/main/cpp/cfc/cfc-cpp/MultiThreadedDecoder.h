...
#ifndef AIRGAP_DEC_TAG
#define AIRGAP_DEC_TAG "AirGapDecoderCPP"
#endif
...
{
	__android_log_print(ANDROID_LOG_INFO, AIRGAP_DEC_TAG, "MultiThreadedDecoder ctor mode=%d path=%s threads=%u", mode_val, data_path.c_str(), _numThreads);
	FountainInit::init();
	_pool.start();
}
...
inline int MultiThreadedDecoder::do_extract(const cv::Mat& mat, cv::Mat& img)
{
	...
	++scanned;
	if ((scanned % 30) == 1) {
		__android_log_print(ANDROID_LOG_INFO, AIRGAP_DEC_TAG, "scan anchors=%zu frame=%dx%d type=%d", anchors.size(), mat.cols, mat.rows, mat.type());
	}
...
inline bool MultiThreadedDecoder::add(cv::Mat mat)
{
    ...
    return _pool.try_execute( [&, mat, modeVal] () {
		try {
			...
			if ((decoded % 20) == 1) {
				__android_log_print(ANDROID_LOG_INFO, AIRGAP_DEC_TAG, "worker decode mode=%u res=%u perfect=%ld decoded=%ld", modeVal, decodeRes, perfect, decoded);
			}
			...
			__android_log_print(ANDROID_LOG_ERROR, AIRGAP_DEC_TAG, "worker cv::Exception: %s", e.what());
		} catch (const std::exception& e) {
			__android_log_print(ANDROID_LOG_ERROR, AIRGAP_DEC_TAG, "worker std::exception: %s", e.what());
		} catch (...) {
			__android_log_print(ANDROID_LOG_ERROR, AIRGAP_DEC_TAG, "worker unknown exception");
		}
	} );
}
